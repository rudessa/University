...
def handle_forget_me(message: telebot.types.Message):
    payload = {"chat_id": message.chat.id, "username": ""}
    response = requests.post(IDENTIFY_USERS_URL, json=payload)
    if response.status_code == 200:
        user_data = response.json()
...
def send_telegram_notification(message: str, owner_chat_id=None, extra_chat_ids=None):
    """Send a notification message to the configured Telegram chats and device owner."""
    targets = chat_ids.copy()
    
    # Add owner's chat ID if specified
    if owner_chat_id is not None:
       targets.append(int(owner_chat_id))
    
    # Add additional chat IDs if specified
    if extra_chat_ids:
        targets.extend([int(chat_id) for chat_id in extra_chat_ids])

    # Send messages to all unique chat IDs
